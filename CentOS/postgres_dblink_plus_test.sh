...
psql -d postgres -c "CREATE USER MAPPING FOR postgres SERVER ipds_db_link OPTIONS (user 'ec_ipds', password 'ec_ipds');"
psql -d postgres -c "GRANT USAGE ON SCHEMA dblink TO postgres;"
psql -d postgres -c "GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA dblink TO postgres;"
psql -d postgres -c "GRANT DELETE ON ALL TABLES IN SCHEMA dblink TO postgres;"
psql -d postgres -c "GRANT USAGE ON FOREIGN SERVER ipds_db_link TO postgres;"
